/*****************************************************************************
* Filename:          C:\Users\superman\Desktop\SpaceInvaders\SpaceInvaders/drivers/dmacontroller_v1_00_a/src/dmacontroller_selftest.c
* Version:           1.00.a
...
static Xuint8 __attribute__((aligned (128))) SrcBuffer[DMACONTROLLER_SELFTEST_BUFSIZE];   /* Source buffer      */
static Xuint8 __attribute__((aligned (128))) DstBuffer[DMACONTROLLER_SELFTEST_BUFSIZE];   /* Destination buffer */
...
XStatus DMACONTROLLER_SelfTest(void * baseaddr_p)
{
  ...
  for (write_loop_index = 0 ; write_loop_index < DMACONTROLLER_USER_NUM_REG; write_loop_index++)
    DMACONTROLLER_mWriteSlaveReg0 (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
  for (read_loop_index = 0 ; read_loop_index < DMACONTROLLER_USER_NUM_REG; read_loop_index++)
    if ( DMACONTROLLER_mReadSlaveReg0 (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
  ...
  if ( (DMACONTROLLER_mReadSlaveReg0 (baseaddr, 0)) != 0x0){
  ...
  * Master */
  ...
  for ( Index = 0; Index < DMACONTROLLER_SELFTEST_BUFSIZE; Index++ )
  {
    SrcBuffer[Index] = Index;
    DstBuffer[Index] = 0;
  }
  xil_printf("   - start user logic master module to receive one word from the source\n\r");
  DMACONTROLLER_MasterRecvWord(baseaddr, (Xuint32) SrcBuffer);
  while ( ! DMACONTROLLER_mMasterDone(baseaddr) ) {}
  xil_printf("   - transfer completed\n\r");
  xil_printf("   - start user logic master module to send onw word to the destination\n\r");
  DMACONTROLLER_MasterSendWord(baseaddr, (Xuint32) DstBuffer);
  while ( ! DMACONTROLLER_mMasterDone(baseaddr) ) {}
  xil_printf("   - transfer completed\n\r");
